...
function SkeletonCard({ wide = false }: { wide?: boolean }) {
  return (
    <div className={`relative overflow-hidden rounded-2xl p-4 md:p-6 border bg-surface border-foreground/10`}>
...
export function LiveMetrics() {
  ...
  return (
    <motion.section
      ...
      className="relative overflow-hidden glass rounded-3xl p-6 md:p-10 text-foreground"
    >
      <div className="relative">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <button onClick={() => setExpanded(!expanded)} className="flex items-center gap-3 group text-left">
            <div className="p-2 rounded-xl bg-white/40 border border-foreground/10">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <div className="p-1 rounded-lg bg-accent-bg group-hover:bg-accent/20 transition-colors">
          ...
          <div className="flex flex-col items-end gap-2">
            <div className="flex items-center gap-2 sm:gap-3">
              <button
                ...
                disabled={refreshing}
                className="group/btn flex items-center gap-2 px-4 py-2.5 rounded-full bg-white/40 border border-foreground/10 hover:bg-accent-bg disabled:opacity-60 disabled:cursor-not-allowed transition-all text-xs font-semibold text-accent"
              >
              ...
              <div className="flex items-center gap-3 px-4 py-2.5 rounded-full bg-white/[0.4] border border-foreground/10">
        ...
        <div className="space-y-4">
          ...
          {metrics && projectGroups.map((group, gi) => {
            const accentMap: Record<string, { border: string; bg: string; icon: string; dot: string; btnBorder: string }> = {
              emerald: { border: "border-l-accent", bg: "bg-white/60 dark:bg-white/[0.04]", icon: "text-accent", dot: "bg-accent-light", btnBorder: "border-foreground/10" },
              purple: { border: "border-l-purple-500", bg: "bg-white/60 dark:bg-white/[0.04]", icon: "text-purple-600", dot: "bg-purple-500", btnBorder: "border-foreground/10" },
              blue: { border: "border-l-blue-500", bg: "bg-white/60 dark:bg-white/[0.04]", icon: "text-blue-600", dot: "bg-blue-500", btnBorder: "border-foreground/10" },
              orange: { border: "border-l-orange-500", bg: "bg-white/60 dark:bg-white/[0.04]", icon: "text-orange-600", dot: "bg-orange-500", btnBorder: "border-foreground/10" },
            };
            ...
            <div
              key={group.name}
              className={`rounded-2xl ${accent.bg} border border-foreground/10 ${accent.border} border-l-2 p-4 md:p-5`}
            >
              <div className="flex items-center justify-between mb-4">
                <Link
                  href={group.link}
                  className="group flex items-center gap-2 text-sm font-semibold text-foreground/80 hover:text-accent transition-colors"
                >
                ...
                <a
                  ...
                  className={`text-[10px] sm:text-xs font-semibold flex items-center gap-1.5 px-3 py-1.5 rounded-full border transition-all bg-white/60 ${accent.btnBorder} ${accent.icon} hover:bg-accent-bg`}
                >
                ...
                {group.cards.map((card, ci) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`relative overflow-hidden rounded-2xl p-4 md:p-6 transition-shadow ${
                        isHero
                          ? "bg-gradient-to-br from-accent to-splash text-white border border-accent/40"
                      ...
                      <div className="relative">
                        <div className="flex items-center gap-1.5 md:gap-2 mb-2 md:mb-3">
                          <div
                            className={`p-1 md:p-1.5 rounded-lg ${
                              isHero ? "bg-white/20" : "bg-accent-bg"
                            }`}
                          >
                            <Icon
                              className={`w-3.5 h-3.5 md:w-4 md:h-4 ${
                                isHero ? "text-white" : "text-muted"
                              }`}
                            />
                          </div>
                        ...
                        <p
                          className={`font-light tracking-tight ${
                            isHero
                              ? "text-2xl sm:text-4xl md:text-5xl text-white"
        ...
        <div className="mt-8 pt-6 border-t border-foreground/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-1 rounded-full border border-foreground/10 bg-foreground/[0.04] text-[10px] font-semibold text-accent uppercase tracking-wider">
              Supabase
            </span>
            <span className="px-2.5 py-1 rounded-full border border-foreground/10 bg-foreground/[0.04] text-[10px] font-semibold text-blue-600 uppercase tracking-wider">
              PostgreSQL
            </span>
            <span className="px-2.5 py-1 rounded-full border border-foreground/10 bg-foreground/[0.04] text-[10px] font-semibold text-muted uppercase tracking-wider">
              4 {t.databases}
            </span>
          </div>
        </div>
