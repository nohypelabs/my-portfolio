        ...
        {services.map((service) => {
          ...
          return (
            <motion.div
              key={service.id}
              variants={fadeInUp}
              className="bg-[#f7f3e8] border border-neutral-200 rounded-xl p-6 hover:border-[#c4956a]/30 hover:shadow-sm transition-all group"
            >
              <div className="flex items-start gap-4">
                <div className="w-11 h-11 rounded-lg bg-[#c4956a]/10 flex items-center justify-center flex-shrink-0 group-hover:bg-[#c4956a]/20 transition-colors">
                  ...
                  <ul className="space-y-1.5">
                    {service.features.map((f, j) => (
                      <li key={j} className="flex items-center gap-2 text-[12px] text-neutral-600">
                        <CheckCircle className="w-3.5 h-3.5 text-[#c4956a] flex-shrink-0" strokeWidth={1.5} />
                        {f}
                      </li>
                    ))}
                  </ul>
