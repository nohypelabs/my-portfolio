    ...
    <motion.div variants={staggerContainer} initial="initial" animate="animate" className="space-y-8">
      <motion.div variants={fadeInUp} className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-[20px] font-semibold text-foreground">Kelola Testimoni</h1>
        </div>
        <button onClick={addTestimonial} className="flex items-center gap-1.5 px-3 py-2 bg-accent-light text-foreground rounded-lg text-[12px] font-medium hover:bg-accent-dark hover:text-surface transition-colors">
      ...
      <div className="space-y-4">
        {testimonials.map((t) => (
          <motion.div key={t.id} variants={fadeInUp} className="bg-surface border border-foreground/20 rounded-[8px] p-5">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                {t.is_featured && <span className="text-[10px] font-medium text-accent bg-accent/10 px-2 py-0.5 rounded-full">Featured</span>}
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => saveTestimonial(t)} disabled={saving === t.id}
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-accent-light text-foreground rounded-lg text-[11px] font-medium hover:bg-accent-dark hover:text-surface transition-colors disabled:opacity-50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
              <input type="text" value={t.name} onChange={e => updateField(t.id, 'name', e.target.value)} placeholder="Nama"
                className="px-3 py-2 border border-foreground/30 rounded-lg text-[12px] focus:outline-none focus:border-accent" />
            ...
            <textarea value={t.content} onChange={e => updateField(t.id, 'content', e.target.value)} rows={2} placeholder="Isi testimoni..."
              className="w-full px-3 py-2 border border-foreground/30 rounded-lg text-[12px] focus:outline-none focus:border-accent resize-none mb-3" />
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2 text-[11px] text-neutral-700 cursor-pointer">
                <input type="checkbox" checked={t.is_featured} onChange={e => updateField(t.id, 'is_featured', e.target.checked)}
                  className="w-3.5 h-3.5 rounded border-foreground/30 text-accent" /> Featured
              </label>
              <label className="flex items-center gap-2 text-[11px] text-neutral-700 cursor-pointer">
                <input type="checkbox" checked={t.is_active} onChange={e => updateField(t.id, 'is_active', e.target.checked)}
                  className="w-3.5 h-3.5 rounded border-foreground/30 text-accent" /> Aktif
              </label>
              <div className="flex items-center gap-1">
                ...
                <select value={t.rating} onChange={e => updateField(t.id, 'rating', parseInt(e.target.value))}
                  className="px-2 py-1 border border-foreground/30 rounded text-[11px] focus:outline-none focus:border-accent">
