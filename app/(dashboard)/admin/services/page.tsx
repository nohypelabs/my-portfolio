...
export default function AdminServicesPage() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center min-h-[60vh]"><Loader2 className="w-6 h-6 animate-spin text-accent" /></div>;
  }
  ...
  return (
    <motion.div variants={staggerContainer} initial="initial" animate="animate" className="space-y-8">
      <motion.div variants={fadeInUp}>
        ...
        <h1 className="text-[20px] font-semibold text-foreground">Kelola Layanan</h1>
      </motion.div>
      ...
      <div className="space-y-6">
        {services.map((svc) => (
          <motion.div key={svc.id} variants={fadeInUp} className="bg-surface border border-foreground/20 rounded-[8px] p-6">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-[15px] font-semibold text-foreground">{svc.title}</h3>
              <button
                onClick={() => saveService(svc)}
                disabled={saving === svc.id}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-accent-light text-foreground rounded-lg text-[12px] font-medium hover:bg-accent-dark hover:text-surface transition-colors disabled:opacity-50"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
              <div>
                ...
                <input type="text" value={svc.title} onChange={e => updateField(svc.id, 'title', e.target.value)}
                  className="w-full px-3 py-2 border border-foreground/30 rounded-lg text-[13px] focus:outline-none focus:border-accent focus:shadow-[4px_4px_0_0_var(--color-accent)]" />
              </div>
              <div>
              ...
              <div className="md:col-span-2">
                ...
                <textarea value={svc.description} onChange={e => updateField(svc.id, 'description', e.target.value)} rows={2}
                  className="w-full px-3 py-2 border border-foreground/30 rounded-lg text-[13px] focus:outline-none focus:border-accent focus:shadow-[4px_4px_0_0_var(--color-accent)] resize-none" />
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 mb-4">
              <label className="flex items-center gap-2 text-[12px] text-neutral-700 cursor-pointer">
                <input type="checkbox" checked={svc.is_active} onChange={e => updateField(svc.id, 'is_active', e.target.checked)}
                  className="w-4 h-4 rounded border-foreground/30 text-accent focus:ring-accent" />
                Aktif
              </label>
              <div>
                ...
                <input type="number" value={svc.sort_order} onChange={e => updateField(svc.id, 'sort_order', parseInt(e.target.value) || 0)}
                  className="w-16 px-2 py-1 border border-foreground/30 rounded-lg text-[12px] focus:outline-none focus:border-accent" />
              </div>
            </div>
                ...
                {svc.features.map((f, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <input type="text" value={f} onChange={e => updateFeature(svc.id, i, e.target.value)}
                      className="flex-1 px-3 py-2 border border-foreground/30 rounded-lg text-[12px] focus:outline-none focus:border-accent focus:shadow-[4px_4px_0_0_var(--color-accent)]" />
                    <button onClick={() => removeFeature(svc.id, i)} className="p-2 text-neutral-400 hover:text-red-500 transition-colors">
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  </div>
                ))}
                <button onClick={() => addFeature(svc.id)} className="flex items-center gap-1.5 text-[12px] text-accent hover:text-accent-dark transition-colors">
